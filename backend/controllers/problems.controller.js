...
const fetchProblems = asyncHandler(async (req, res) => {
    ...
    const APIresponse = await axios.get(`https://codeforces.com/api/user.status?handle=${handle}&from=1&count=10000`);
    ...
    for (const response of responses) {
        ...
        const existingProblem = await Problems.findOne({ contestId, index, solver });
